...
fn run_ukf(input_file: &File, output_file: &File) -> Result<(), String> {
    ...
    let mut wtr = csv::WriterBuilder::new().delimiter(b'\t').from_writer(output_file);
    ...
    for (i, m) in measurements.iter().enumerate() {
        ...
        let (px_meas, py_meas) = match m.sensor_type {
                SensorType::Lidar => {
                    m.lidar_data.as_ref().unwrap().point()
                }
                SensorType::Radar => {
                    m.radar_data.as_ref().unwrap().point()
                }
            };

        let gtp = &ground_truths[i];
        let output = Output {px_est: x[0], py_est: x[1], vel_abs_est: x[2], yaw_angle_est: x[3], yaw_rate_est: x[4],
                             px_meas: px_meas, py_meas: py_meas, 
                             px_gt: gtp.x, py_gt: gtp.y, vx_gt: gtp.vx, vy_gt: gtp.vy,
                             nis_lidar: ukf.nis_lidar, nis_radar: ukf.nis_radar}; 
        wtr.serialize(output).unwrap();
    }

    info!("Accruacry - RMSE: {:?}", util::helper::calculate_rmse(&estimations, &ground_truths));
    wtr.flush().unwrap();
